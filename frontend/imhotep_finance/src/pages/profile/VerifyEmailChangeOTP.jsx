...
const VerifyEmailChangeOTP = () => {
    ...
    useEffect(() => {
        if (location.state?.email && location.state?.new_email) {
            setEmail(location.state.email);
            setNewEmail(location.state.new_email);
        } else {
            setError('Missing email information. Please try updating your profile again.');
        }
    }, [location.state]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/api/profile/verify-email-change-otp/', {
                email: email,
                new_email: newEmail,
                otp: otp
            });
            ...
            setSuccess(true);
            // Logout user as they need to re-login with new email
            setTimeout(async () => {
                await logout();
                navigate('/login', {
                    state: { message: 'Email updated successfully! Please log in with your new email.' }
                });
            }, 3000);
                    ...
                    <div className="relative w-full max-w-md">
                        <div className="chef-card rounded-3xl p-8 shadow-2xl backdrop-blur-2xl text-center">
                            ...
                            <p className="text-gray-600 dark:text-gray-300 font-medium mb-8">
                                Your email has been successfully updated. Redirecting to login...
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="chef-card rounded-3xl p-8 shadow-2xl backdrop-blur-2xl">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    value={email}
                                    disabled
                                    className="chef-input w-full bg-gray-100 dark:bg-gray-700 cursor-not-allowed"
                                />
                            </div>
                        ...
                        <div className="mt-8 text-center">
                            <p className="text-gray-600 dark:text-gray-300 text-sm">
                                <Link to="/profile" className="font-semibold transition-colors hover:underline" style={{ color: '#366c6b' }}>
